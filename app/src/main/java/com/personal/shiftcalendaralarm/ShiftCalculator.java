...
public class ShiftCalculator {
    public static ShiftResult calculate(DatabaseHelper db, LocalDate date) {
        ...
        ShiftType conditionalShift = findConditionalShift(db, calculatedShift, date);
    ...
    private static ShiftType findConditionalShift(DatabaseHelper db, ShiftType calculatedShift, LocalDate date) {
        ...
        for (ShiftType type : db.getAutoConditionShiftTypes()) {
            ...
            if (!matchesHolidayFilter(db, type.conditionHolidayFilter, date)) continue;
