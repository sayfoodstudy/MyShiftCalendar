...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        createAlarmFoldersTable(db);
        createAlarmConditionsTable(db);
    ...
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        ...
        if (oldVersion < 6) {
            addAlarmFolderAndOrderColumns(db);
            createAlarmFoldersTable(db);
            createAlarmConditionsTable(db);
        }
        if (oldVersion < 7) {
            addAlarmConditionMultiShiftColumn(db);
        }
        if (oldVersion < 2) {
    ...
    private void addAlarmFolderAndOrderColumns(SQLiteDatabase db) {
        try { db.execSQL("ALTER TABLE alarms ADD COLUMN folder_id INTEGER NOT NULL DEFAULT 0"); } catch (Exception ignored) { }
        try { db.execSQL("ALTER TABLE alarms ADD COLUMN sort_order INTEGER NOT NULL DEFAULT 0"); } catch (Exception ignored) { }
        try { db.execSQL("UPDATE alarms SET sort_order=id WHERE sort_order=0"); } catch (Exception ignored) { }
    }

    private void createAlarmFoldersTable(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS alarm_folders (" +
                "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
                "name TEXT NOT NULL, " +
                "sort_order INTEGER NOT NULL DEFAULT 0" +
                ")");
    ...
    private void createAlarmConditionsTable(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS alarm_conditions (" +
                "id INTEGER PRIMARY KEY AUTOINCREMENT, " +
    ...
    private void addAlarmConditionMultiShiftColumn(SQLiteDatabase db) {
        try { db.execSQL("ALTER TABLE alarm_conditions ADD COLUMN shift_type_ids TEXT"); } catch (Exception ignored) { }
        try { db.execSQL("UPDATE alarm_conditions SET shift_type_ids=CAST(shift_type_id AS TEXT) WHERE shift_type_ids IS NULL OR shift_type_ids=''"); } catch (Exception ignored) { }
    }
    ...
    public boolean isPublicOrCustomHoliday(LocalDate date) {
        return HolidayProvider.isPublicHoliday(date) || getCustomHolidayName(date) != null;
    }
    ...
    public long addAlarm(String title, String memo, String alarmMode, int hour, int minute,
        ...
        values.put("folder_id", AlarmItem.FOLDER_NONE);
        values.put("sort_order", getNextAlarmSortOrder(db));
        long id = db.insert("alarms", null, values);
        if (id > 0 && AlarmItem.MODE_SHIFT.equals(alarmMode)) {
            replaceAlarmConditions(id, java.util.Collections.singletonList(
                    new AlarmCondition(-1L, id, 1, shiftTypeId, holidayFilter, weekdayMask)));
        }
        return id;
    }

    private int getNextAlarmSortOrder(SQLiteDatabase db) {
        Cursor cursor = db.rawQuery("SELECT COALESCE(MAX(sort_order), 0) + 10 FROM alarms", null);
        try {
            if (cursor.moveToFirst()) return cursor.getInt(0);
            return 10;
        } finally {
    ...
    public List<AlarmItem> getAllAlarms() {
        ...
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query("alarms", null, null, null, null, null, "sort_order ASC, id ASC");
        try {
    ...
    public List<AlarmItem> getAlarmsInFolder(long folderId) {
        List<AlarmItem> result = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query("alarms", null, "folder_id=?", new String[]{String.valueOf(folderId)},
    ...
    public List<AlarmItem> getEnabledAlarms() {
        List<AlarmItem> result = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query("alarms", null, "enabled=1", null, null, null, "sort_order ASC, id ASC");
        try {
            while (cursor.moveToNext()) result.add(alarmFromCursor(cursor));
        } finally {
            cursor.close();
    ...
    public void deleteAlarm(long id) {
        SQLiteDatabase db = getWritableDatabase();
        db.delete("alarm_conditions", "alarm_id=?", new String[]{String.valueOf(id)});
        db.delete("alarms", "id=?", new String[]{String.valueOf(id)});
    }

    public void updateAlarmFolder(long alarmId, long folderId) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
    ...
    public boolean swapAlarmOrder(long firstAlarmId, long secondAlarmId) {
        if (firstAlarmId <= 0 || secondAlarmId <= 0 || firstAlarmId == secondAlarmId) return false;
        AlarmItem first = getAlarm(firstAlarmId);
        AlarmItem second = getAlarm(secondAlarmId);
        if (first == null || second == null) return false;
        SQLiteDatabase db = getWritableDatabase();
        ContentValues a = new ContentValues();
        a.put("sort_order", second.sortOrder);
        db.update("alarms", a, "id=?", new String[]{String.valueOf(first.id)});
        ContentValues b = new ContentValues();
        b.put("sort_order", first.sortOrder);
        db.update("alarms", b, "id=?", new String[]{String.valueOf(second.id)});
        return true;
    }

    public boolean moveAlarmOrder(long alarmId, int direction) {
        SQLiteDatabase db = getWritableDatabase();
        AlarmItem current = getAlarm(alarmId);
        if (current == null) return false;
        String op = direction < 0 ? "<" : ">";
        String order = direction < 0 ? "sort_order DESC, id DESC" : "sort_order ASC, id ASC";
        Cursor cursor = db.query("alarms", null, "sort_order " + op + " ?",
                new String[]{String.valueOf(current.sortOrder)}, null, null, order, "1");
        try {
            if (!cursor.moveToFirst()) return false;
            AlarmItem other = alarmFromCursor(cursor);
            ContentValues a = new ContentValues();
            a.put("sort_order", other.sortOrder);
            db.update("alarms", a, "id=?", new String[]{String.valueOf(current.id)});
            ContentValues b = new ContentValues();
            b.put("sort_order", current.sortOrder);
            db.update("alarms", b, "id=?", new String[]{String.valueOf(other.id)});
            return true;
        } finally {
            cursor.close();
        }
    ...
    public String exportBackupJson() throws Exception {
        ...
        root.put("alarm_folders", tableToJson(db, "alarm_folders"));
        root.put("alarm_conditions", tableToJson(db, "alarm_conditions"));
    ...
    public void restoreBackupJson(String json) throws Exception {
        ...
        db.beginTransaction();
        try {
            db.delete("alarm_conditions", null, null);
            db.delete("alarm_folders", null, null);
            ...
            restoreTable(db, "alarm_folders", root.optJSONArray("alarm_folders"));
            restoreTable(db, "alarm_conditions", root.optJSONArray("alarm_conditions"));
    ...
    public List<AlarmFolder> getAlarmFolders() {
        List<AlarmFolder> result = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query("alarm_folders", null, null, null, null, null, "sort_order ASC, id ASC");
        try {
            while (cursor.moveToNext()) result.add(folderFromCursor(cursor));
        } finally {
            cursor.close();
        }
        return result;
    ...
    public long addAlarmFolder(String name) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("name", name == null || name.trim().isEmpty() ? "새 폴더" : name.trim());
        values.put("sort_order", getNextFolderSortOrder(db));
        return db.insert("alarm_folders", null, values);
    }

    private int getNextFolderSortOrder(SQLiteDatabase db) {
        Cursor cursor = db.rawQuery("SELECT COALESCE(MAX(sort_order), 0) + 10 FROM alarm_folders", null);
        try {
            if (cursor.moveToFirst()) return cursor.getInt(0);
            return 10;
        } finally {
    ...
    public void deleteAlarmFolder(long folderId) {
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put("folder_id", AlarmItem.FOLDER_NONE);
        db.update("alarms", values, "folder_id=?", new String[]{String.valueOf(folderId)});
        db.delete("alarm_folders", "id=?", new String[]{String.valueOf(folderId)});
    }

    private AlarmFolder folderFromCursor(Cursor cursor) {
        return new AlarmFolder(
                cursor.getLong(cursor.getColumnIndexOrThrow("id")),
                cursor.getString(cursor.getColumnIndexOrThrow("name")),
                cursor.getInt(cursor.getColumnIndexOrThrow("sort_order"))
        );
    ...
    public List<AlarmCondition> getAlarmConditions(long alarmId) {
        List<AlarmCondition> result = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        Cursor cursor = db.query("alarm_conditions", null, "alarm_id=?",
                new String[]{String.valueOf(alarmId)}, null, null, "group_order ASC, id ASC");
        try {
            while (cursor.moveToNext()) result.add(conditionFromCursor(cursor));
        } finally {
            cursor.close();
        }
        return result;
    ...
    public void replaceAlarmConditions(long alarmId, List<AlarmCondition> conditions) {
        SQLiteDatabase db = getWritableDatabase();
        db.delete("alarm_conditions", "alarm_id=?", new String[]{String.valueOf(alarmId)});
        if (conditions == null || conditions.isEmpty()) return;
        int order = 1;
        for (AlarmCondition condition : conditions) {
            ContentValues values = new ContentValues();
            values.put("alarm_id", alarmId);
            values.put("group_order", order++);
            values.put("shift_type_id", condition.shiftTypeId);
            values.put("shift_type_ids", condition.shiftTypeIds);
            values.put("holiday_filter", condition.holidayFilter);
            values.put("weekday_mask", condition.weekdayMask);
            db.insert("alarm_conditions", null, values);
        }
    }

    private AlarmCondition conditionFromCursor(Cursor cursor) {
        long shiftId = cursor.getLong(cursor.getColumnIndexOrThrow("shift_type_id"));
        int idsIndex = cursor.getColumnIndex("shift_type_ids");
        String shiftIds = idsIndex >= 0 && !cursor.isNull(idsIndex) ? cursor.getString(idsIndex) : String.valueOf(shiftId);
        return new AlarmCondition(
                cursor.getLong(cursor.getColumnIndexOrThrow("id")),
                cursor.getLong(cursor.getColumnIndexOrThrow("alarm_id")),
                cursor.getInt(cursor.getColumnIndexOrThrow("group_order")),
                shiftId,
                shiftIds,
    ...
    private long getLongColumn(Cursor cursor, String name, long defaultValue) {
        int index = cursor.getColumnIndex(name);
        if (index < 0 || cursor.isNull(index)) return defaultValue;
        return cursor.getLong(index);
    }

    private int getIntColumn(Cursor cursor, String name, int defaultValue) {
        int index = cursor.getColumnIndex(name);
        if (index < 0 || cursor.isNull(index)) return defaultValue;
    ...
    private AlarmItem alarmFromCursor(Cursor cursor) {
        return new AlarmItem(
                ...
                getLongColumn(cursor, "folder_id", AlarmItem.FOLDER_NONE),
                getIntColumn(cursor, "sort_order", (int) cursor.getLong(cursor.getColumnIndexOrThrow("id")))
        );
    }
}
