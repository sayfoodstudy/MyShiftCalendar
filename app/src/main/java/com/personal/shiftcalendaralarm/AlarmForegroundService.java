...
public class AlarmForegroundService extends Service {
    ...
    public static final String PREFS_NAME = "shift_alarm_prefs";
    public static final String KEY_MISSED_ALARM_TITLE = "missed_alarm_title";
    public static final String KEY_MISSED_ALARM_TIME = "missed_alarm_time";

    private static final long AUTO_STOP_DELAY_MS = 60_000L;
    ...
    private final Runnable autoStopRunnable = () -> {
        saveMissedAlarmInfo(currentAlarmTitle);
        stopAlarmSound();
    ...
    private void startAlarm(long alarmId) {
        ...
        currentAlarmTitle = alarm.title == null || alarm.title.trim().isEmpty() ? "알람" : alarm.title;
        ...
        showAlarmScreen(alarm.id);
        autoStopHandler.removeCallbacks(autoStopRunnable);
        autoStopHandler.postDelayed(autoStopRunnable, AUTO_STOP_DELAY_MS);
    }

    private void saveMissedAlarmInfo(String title) {
        try {
            SharedPreferences prefs = getSharedPreferences(PREFS_NAME, MODE_PRIVATE);
            String timeText = new SimpleDateFormat("a h:mm", Locale.KOREAN).format(new Date());
            prefs.edit()
                    .putString(KEY_MISSED_ALARM_TITLE, title == null || title.isEmpty() ? "알람" : title)
                    .putString(KEY_MISSED_ALARM_TIME, timeText)
                    .apply();
        } catch (Exception ignored) { }
    }

    private void showAlarmScreen(long alarmId) {
        try {
